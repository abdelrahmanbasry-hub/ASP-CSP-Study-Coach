...
export function InteractiveHazardScene({ config, name, language, consequences = [], selectedOverlayId, onSelectOverlay }: HazardSceneProps) {
  ...
  if (!template || !supportsSceneEngine(template, config.kind)) return <section className="hazard-scene scene-unavailable" data-visualization-engine={config.kind}>
    <h3>{text({ en: "Scene template unavailable", ar: "الرسم التوضيحي غير متاح" })}</h3><p>{text(config.description)}</p>
  </section>;
  ...
  return <section className="hazard-scene" data-visualization-engine={config.kind} data-template={config.template} data-mode={mode} aria-label={bilingualLabel(engine.name, language)}>
    ...
    <div className="scene-stage" dir="ltr">
      ...
      {imageFailed && <p className="scene-image-error">{text({ en: "Illustration unavailable. All scene descriptions remain accessible below.", ar: "تعذّر عرض الرسم، ويمكنك متابعة جميع الأوصاف أدناه." })}</p>}
      ...
      <div className="scene-markers" role="group" aria-label={language === "ar" ? "النقاط التوضيحية" : "Scene points"}>
    ...
    <p className="scene-scale-note">{text({ en: "Illustrative positions and zones · not to scale", ar: "المواضع والمناطق تقريبية · الرسم غير مقياسي" })}</p>
    <div className="scene-modes" role="group" aria-label={language === "ar" ? "طرق عرض الرسم" : "Scene modes"}>{modes.map(({ id, label, icon: Icon }) => <button key={id} onKeyDown={navigateGroup} type="button" aria-pressed={mode === id} onClick={() => setMode(id)}><Icon size={18} aria-hidden="true" />{text(label)}</button>)}</div>
    <div className="scene-callout-heading"><h4>{text(mode === "effects" ? { en: "Effects & affected areas", ar: "العواقب والمناطق المتأثرة" } : config.kind === "concept-diagram" ? { en: "Core principles", ar: "المبادئ الأساسية" } : { en: "Explore the scene", ar: "استكشف الرسم" })}</h4>{selected && <button type="button" onClick={() => select(null)}>{text({ en: "Clear selection", ar: "إزالة التحديد" })}</button>}</div>
    {mode === "effects" && <div className="scene-effects" role="status">{text({ en: "Hazard-level effects; select a callout for its specific context.", ar: "هذه هي العواقب العامة للخطر. اختر علامة توضيحية لعرض تفاصيلها." })}{consequences.map((value, i) => <p key={i}>{text(value)}</p>)}</div>}
    <div className="scene-selection-summary" role="status" aria-live="polite">{selected ? <><strong>{text(selected.label)}</strong><p>{text(selected.description)}</p>{mode === "effects" && selected.consequences?.map((value, index) => <p key={index}>{text(value)}</p>)}</> : <p><Info size={15} aria-hidden="true" />{text({ en: "Select a numbered point or callout to focus the scene and related details.", ar: "اختر رقمًا أو علامة توضيحية لعرض تفاصيل هذا الجزء من الرسم." })}</p>}</div>
    <div className="scene-callouts" role="group" aria-label={language === "ar" ? "العلامات التوضيحية" : "Scene callouts"} dir={language === "ar" ? "rtl" : "ltr"}>
