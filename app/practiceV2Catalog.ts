...
import { isPracticeV2ProductionEligible, validatePracticeV2Pack, type PracticeV2Question, type PracticeV2QuestionPack } from "./practiceV2.ts";
...
const verifiedImportedQuestions = importedPacks
  .flatMap((pack) => pack.questions)
  .filter(isPracticeV2ProductionEligible);
...
export const PRACTICE_V2_HAS_VERIFIED_CONTENT = verifiedImportedQuestions.length > 0;
export const PRACTICE_V2_QUESTIONS: readonly PracticeV2Question[] = process.env.NODE_ENV === "production"
  ? verifiedImportedQuestions
  : [...verifiedImportedQuestions, ...demoQuestions];
